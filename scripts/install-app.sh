...
APP_SUPPORT="$HOME/Library/Application Support/com.ping.codex-account-switch-launcher"
...
mkdir -p "$APP_SUPPORT"
if [[ ! -f "$APP_SUPPORT/launcher.sqlite3" && -f "$OLD_PROJECT_DATA/launcher.sqlite3" ]]; then
  echo "Migrating existing launcher data to Application Support..."
...
<plist version="1.0"><dict>
  <key>CFBundleName</key><string>Codex账号切换启动器</string>
  <key>CFBundleDisplayName</key><string>Codex账号切换启动器</string>
  <key>CFBundleIdentifier</key><string>com.ping.codex-account-switch-launcher</string>
...
echo "Installed standalone native App: $APP_DIR"
