...
- 自动预热已绑定账号的 5 小时窗口，不切换或打断前台 ChatGPT。

## 检查模式

在设置中打开 **检查模式**，开关会自动保存。左侧保留当前启动台，右侧按建议优先展示其他启动台的紧凑账号行：周剩余、周重置时间、5 小时剩余、5 小时重置时间和可重置次数。每行沿用启动台颜色，并保留账号名称与“启动”按钮。

宽窗口按当前卡片高度容纳最多 5 行，放不下的行自动移到下方“启动台”区域；没有多余行时隐藏下方区域。窄窗口自动改为上下排列。全局立即同步可刷新已经绑定的账号，无需等待所有账号绑定。

日常可在 A 内切换账号登录；紧凑行的“启动”仍然打开对应启动台，用于绑定或维护账号。检查模式不会自动将其他启动台的登录凭据搬到 A。关闭检查模式即可恢复完整卡片和最近切换记录。
...
已绑定的真实账号默认开启自动预热。Codex Switcher 每小时统一同步一次所有已绑定账号；已确认的 `prime_next_at` 同时作为持久化的下次 5H 重置任务。一个统一的后台循环每 5 分钟批量处理所有到期重置、延迟校验和校验重试。当需要启动新窗口且账号位于配置的活跃时段内时，它会通过对应 `CODEX_HOME` 启动一个临时、只读的 Codex CLI 会话，使用低消耗模型发送一道随机的两位数加法（两个加数及结果均不超过 100）。请求后的重置时间只作为候选值；至少 5 分钟后再次同步，只有绝对时间误差不超过 5 秒且剩余倒计时已低于 4 小时 59 分钟时才确认成功。网络或校验异常会在后续 5 分钟任务中只重试额度同步，不会再次发送请求。

- 默认活跃时段为全天 24 小时；可在每个账户助记符的编辑面板中独立设置，支持跨午夜。
- 每个账号可独立关闭自动预热。
- 周剩余额度低于 5% 时自动暂停。
- Mac 睡眠时遵循系统策略暂停；唤醒后会立即补做一次检查。
- 命令使用 Codex 的 `--ephemeral` 模式，不持久化 Session，因此不会出现在正常任务列表中，也不需要额外归档；它不会退出、切换或抢占当前 ChatGPT Desktop。
- Codex Switcher 必须保持运行；完全退出应用后后台检查也会停止。
...
安装脚本会先关闭正在运行的 Codex Switcher，再执行正式 Release 构建；用于发布的
ZIP 仍会生成，但本地安装不会解压 ZIP，而是将 `build/release/` 中刚生成的 App
直接移动到 `/Applications`。脚本还会移除个人 Applications 目录中的重复副本，
最后启动安装后的 App。安装成功后，`build/` 中不会保留 App Bundle。
