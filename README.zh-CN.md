...
### v0.22.3 绑定额度只读与 UI 微调

- 已绑定真实账号后，周剩余和 5 小时剩余进度条改为只读；后端也拒绝绕过 UI 手动修改，额度应通过“立即同步”更新。
- 自绘额度滑块轨道和滑块圆点，App 失焦时仍保持启动台主题色，不再被 macOS/WebKit 自动灰化。
- “建议下一个”的低额度警告降低红色饱和度、边框和背景对比度，只保留轻度提醒。
- 下方启动台卡片把“已绑定”和 Plus / Pro 套餐徽标固定为一个紧邻的右侧徽标组。
- 增加“可重置次数”和下方分割线之间的留白，使上下间距更均衡。
